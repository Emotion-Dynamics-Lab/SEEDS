...
Participant 672e088407bba7b95127376e responded 'niether cisgender nor transgender' to gender trajectory question and then "Iam a male" in self describe. Changed both columns to NA
```{r}

demo_scale <- demo_scale %>%
  mutate(
    GenderTrajectoryNeitherCisgenderNorTransgenderText = if_else(id == "672e088407bba7b95127376e" & GenderTrajectoryNeitherCisgenderNorTransgenderText == "Iam a male",
      NA, GenderTrajectoryNeitherCisgenderNorTransgenderText),
    GenderTrajectory = 
      if_else(id == "672e088407bba7b95127376e" & GenderTrajectory == "neither_transgender_nor_cisgender",
      NA, 
      GenderTrajectory))

```

Participant 650bbd883de9e376c0d0d7cd responded 'niether cisgender nor transgender' to gender trajectory question and then "Straight" in self describe. Changed both columns to NA
```{r}
demo_scale <- demo_scale %>%
  mutate(
    GenderTrajectoryNeitherCisgenderNorTransgenderText = if_else(id == "650bbd883de9e376c0d0d7cd" & GenderTrajectoryNeitherCisgenderNorTransgenderText == "Straight",
      NA, GenderTrajectoryNeitherCisgenderNorTransgenderText),
    GenderTrajectory = 
      if_else(id == "650bbd883de9e376c0d0d7cd" & GenderTrajectory == "neither_transgender_nor_cisgender",
      NA, 
      GenderTrajectory))
```
...
Make column for GSM variable (Option 1):
```{r}
demo_scale <- demo_scale %>%
  add_column(GSM = if_else(.$GenderSelfDescribeText %in% "woman" | "man" &
                             .$GenderTrajectory %in% "cisgender" &
                             .$GenderBinaryRelation %in% "binary" &
                             .$SexualOrientation %in% "straight/heterosexual", FALSE, TRUE), .after = "id")
...
Option 2: Calculate a variable indicating sexual and gender minority status. We want to code everyone who indicated both cisgender, binary, and heterosexual into `0` and everyone else into `1`.
```{r}
...
# Now create a new variable that sums these three minority variables
demo_scale$gender_trajectory_minority <- as.numeric(demo_scale$gender_trajectory_minority)
demo_scale$gender_binary_minority <- as.numeric(demo_scale$gender_binary_minority)
demo_scale$sexual_minority <- as.numeric(demo_scale$sexual_minority)

demo_scale$sgm <- demo_scale$gender_trajectory_minority + demo_scale$gender_binary_minority + demo_scale$sexual_minority

# Recode values of 4 = 1 and 3 = 1 and 2 = 1 into this new sgm variable so that 1 =  sexual/gender minority, 0 = not
demo_scale$sgm <- car::recode(demo_scale$sgm, "4 = 1")
demo_scale$sgm <- car::recode(demo_scale$sgm, "3 = 1")
demo_scale$sgm <- car::recode(demo_scale$sgm, "2 = 1")
demo_scale$sgm <- as.numeric(demo_scale$sgm)
...
demo_core <- subset(demo_scale, select = c(id, GSM, GenderIdentity, SexualOrientation, Ethnicity, Age, ParentalEducation, Ladder, InternationalDomesticStudent, UndergradYear, PartOrFullTime, Faculty, GPA))
