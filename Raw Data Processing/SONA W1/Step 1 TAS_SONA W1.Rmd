...
## Edit it so data is only imputed for Ps who responded to at least 50% of the items
...
tas_scale.na <- subset(tas_scale.na, select= -c(id, tas_sum, tas_mean)) #Note from Jess: remove subscales too
...
```{r}
# Create columns in tas_scale that we will merge the mean-imputed data into
tas_scale2$tas_sum_MI <-tas_scale2$tas_sum
tas_scale2$tas_mean_MI <-tas_scale2$tas_mean

# Create empty columns in tas_scale.na for the conventional mean and sum (not MI)
tas_scale.na$tas_sum <-NA
tas_scale.na$tas_mean <-NA

# Order the columns to be consistent for merge
col_order <- c("id", "tas_1", "tas_2", "tas_3", "tas_4", "tas_5", "tas_6", "tas_7", "tas_8", "tas_9", "tas_10","tas_11","tas_12", "tas_13","tas_14","tas_15", "tas_16", "tas_17", "tas_18", "tas_19","tas_20","tas_sum",  "tas_mean", "tas_sum_MI",  "tas_mean_MI", "tas_diff_id_sum", "tas_diff_id_mean", "tas_diff_describ_sum", "tas_diff_describ_mean", "tas_ext_think_sum", "tas_ext_think_mean")

tas_scale2 <- tas_scale2[, col_order]
tas_scale.na <- tas_scale.na[, col_order]

# Merge the imputed cases back into tas_scale
tas_scale3 <- rbind(tas_scale2, tas_scale.na)

# Bring in the non-imputed mean and sum from df_scale, as well as subscale mean and sums
tas_scale3 <- merge(tas_scale3,
                   tas_scale[, c("id", "tas_sum", "tas_mean", "tas_diff_id_sum","tas_diff_id_mean", "tas_diff_describ_sum", "tas_diff_describ_mean", "tas_ext_think_sum", "tas_ext_think_mean")],
                   by = "id",
                   all.x = TRUE,
                   suffixes = c("", "_orig"))
