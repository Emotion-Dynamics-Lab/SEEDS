...
#MI
# Impute missing values for participants who responded to at least 50% of the sub-scale items (2 items).
# ress_4 + ress_9 + ress_13 + ress_18

# First, pull out columns we do not want used in the mean imputation:
# Pull out the id column:
df_scale.na_supp <- df_scale
id.list <- df_scale.na_supp$id
#Remove columns we don't want to include in the mean imputation:
df_scale.na_supp <- subset(df_scale.na_supp, select = -c(id, ress_1:ress_3, ress_5:ress_8, ress_10:ress_12, ress_14:ress_17, ress_19:ress_24, ress_rumination_sum, ress_rumination_mean, ress_rumination_sum_MI, ress_rumination_mean_MI, ress_engagement_mean, ress_engagement_sum, ress_engagement_mean_MI, ress_engagement_sum_MI, ress_reappraisal_mean, ress_reappraisal_sum, ress_reappraisal_mean_MI, ress_reappraisal_sum_MI, ress_suppression_mean, ress_suppression_sum))

#Impute the means, using the approach for imputing row means described [here](https://statisticsglobe.com/mean-imputation-for-missing-data/):

# Count the number of non-missing items per participant
num_items <- ncol(df_scale.na_supp)
min_items_needed <- ceiling(num_items * 0.5)  # at least 50% answered

# Impute row means only for participants who answered >= 50% of items
for(i in 1:nrow(df_scale.na_supp)) {
  non_missing_count <- sum(!is.na(df_scale.na_supp[i, ]))
  
if(non_missing_count >= min_items_needed) {
    # Impute missing values with row mean
    df_scale.na_supp[i, ][is.na(df_scale.na_supp[i, ])] <- mean(as.numeric(df_scale.na_supp[i, ]), na.rm = TRUE)
  } else {
    # If less than 50% answered, set all values to NA
    df_scale.na_supp[i, ] <- NA
  }
}

#Round the values back to whole numbers:
df_scale.na_supp <- round(df_scale.na_supp, digits=0)

#Calculate the sum and mean, using the suffix _MI to indicate that these columns have had means imputed:
df_scale.na_supp$ress_suppression_sum_MI <- apply(df_scale.na_supp[,paste0(c("ress_4", "ress_9", "ress_13", "ress_18"))], 1, sum)
df_scale.na_supp$ress_suppression_mean_MI <- apply(df_scale.na_supp[,paste0(c("ress_4", "ress_9", "ress_13", "ress_18"))], 1, mean)

#Put the IDs back into the data frame:
df_scale.na_supp$id <- id.list

#Put the MI columns back into the original data frame
df_scale$ress_suppression_mean_MI <- df_scale.na_supp$ress_suppression_mean_MI
df_scale$ress_suppression_sum_MI  <- df_scale.na_supp$ress_suppression_sum_MI
...
#MI
# Impute missing values for participants who responded to at least 50% of the sub-scale items (2 items).
# ress_6 + ress_16 + ress_19 + ress_24

# First, pull out columns we do not want used in the mean imputation:
# Pull out the id column:
df_scale.na_rlx <- df_scale
id.list <- df_scale.na_rlx$id

#Remove columns we don't want to include in the mean imputation:
df_scale.na_rlx <- subset(df_scale.na_rlx, select = -c(id, ress_1:ress_5, ress_7:ress_15, ress_17, ress_18, ress_20:ress_23, ress_rumination_sum, ress_rumination_mean, ress_rumination_sum_MI, ress_rumination_mean_MI, ress_engagement_mean, ress_engagement_sum, ress_engagement_mean_MI, ress_engagement_sum_MI, ress_reappraisal_mean, ress_reappraisal_sum, ress_reappraisal_mean_MI, ress_reappraisal_sum_MI, ress_suppression_mean, ress_suppression_sum, ress_suppression_mean_MI, ress_suppression_sum_MI, ress_relaxation_sum, ress_relaxation_mean))

#Impute the means, using the approach for imputing row means described [here](https://statisticsglobe.com/mean-imputation-for-missing-data/):

# Count the number of non-missing items per participant
num_items <- ncol(df_scale.na_rlx)
min_items_needed <- ceiling(num_items * 0.5)  # at least 50% answered

# Impute row means only for participants who answered >= 50% of items
for(i in 1:nrow(df_scale.na_rlx)) {
  non_missing_count <- sum(!is.na(df_scale.na_rlx[i, ]))
  
if(non_missing_count >= min_items_needed) {
    # Impute missing values with row mean
    df_scale.na_rlx[i, ][is.na(df_scale.na_rlx[i, ])] <- mean(as.numeric(df_scale.na_rlx[i, ]), na.rm = TRUE)
  } else {
    # If less than 50% answered, set all values to NA
    df_scale.na_rlx[i, ] <- NA
  }
}

#Round the values back to whole numbers:
df_scale.na_rlx <- round(df_scale.na_rlx, digits=0)

#Calculate the sum and mean, using the suffix _MI to indicate that these columns have had means imputed:
df_scale.na_rlx$ress_relaxation_sum_MI <- apply(df_scale.na_rlx[,paste0(c("ress_6", "ress_16", "ress_19", "ress_24"))], 1, sum)
df_scale.na_rlx$ress_relaxation_mean_MI <- apply(df_scale.na_rlx[,paste0(c("ress_6", "ress_16", "ress_19", "ress_24"))], 1, mean)

#Put the IDs back into the data frame:
df_scale.na_rlx$id <- id.list

#Put the MI columns back into the original data frame
df_scale$ress_relaxation_mean_MI <- df_scale.na_rlx$ress_relaxation_mean_MI
df_scale$ress_relaxation_sum_MI  <- df_scale.na_rlx$ress_relaxation_sum_MI
...
#MI
# Impute missing values for participants who responded to at least 50% of the sub-scale items (2 items).
# ress_7 + ress_10 + ress_17 + ress_22

# First, pull out columns we do not want used in the mean imputation:
# Pull out the id column:
df_scale.na_dis <- df_scale
id.list <- df_scale.na_dis$id

#Remove columns we don't want to include in the mean imputation:
df_scale.na_dis <- subset(df_scale.na_dis, select = -c(id, ress_1:ress_6, ress_8, ress_9, ress_11:ress_16, ress_18:ress_21, ress_23, ress_24, ress_rumination_sum, ress_rumination_mean, ress_rumination_sum_MI, ress_rumination_mean_MI, ress_engagement_mean, ress_engagement_sum, ress_engagement_mean_MI, ress_engagement_sum_MI, ress_reappraisal_mean, ress_reappraisal_sum, ress_reappraisal_mean_MI, ress_reappraisal_sum_MI, ress_suppression_mean, ress_suppression_sum, ress_suppression_mean_MI, ress_suppression_sum_MI, ress_relaxation_sum, ress_relaxation_mean, ress_relaxation_sum_MI, ress_relaxation_mean_MI, ress_distraction_mean, ress_distraction_sum))

#Impute the means, using the approach for imputing row means described [here](https://statisticsglobe.com/mean-imputation-for-missing-data/):

# Count the number of non-missing items per participant
num_items <- ncol(df_scale.na_dis)
min_items_needed <- ceiling(num_items * 0.5)  # at least 50% answered

# Impute row means only for participants who answered >= 50% of items
for(i in 1:nrow(df_scale.na_dis)) {
  non_missing_count <- sum(!is.na(df_scale.na_dis[i, ]))
  
if(non_missing_count >= min_items_needed) {
    # Impute missing values with row mean
    df_scale.na_dis[i, ][is.na(df_scale.na_dis[i, ])] <- mean(as.numeric(df_scale.na_dis[i, ]), na.rm = TRUE)
  } else {
    # If less than 50% answered, set all values to NA
    df_scale.na_dis[i, ] <- NA
  }
}

#Round the values back to whole numbers:
df_scale.na_dis <- round(df_scale.na_dis, digits=0)

#Calculate the sum and mean, using the suffix _MI to indicate that these columns have had means imputed:
df_scale.na_dis$ress_distraction_sum_MI <- apply(df_scale.na_dis[,paste0(c("ress_7", "ress_10", "ress_17", "ress_22"))], 1, sum)
df_scale.na_dis$ress_distraction_mean_MI <- apply(df_scale.na_dis[,paste0(c("ress_7", "ress_10", "ress_17", "ress_22"))], 1, mean)

#Put the IDs back into the data frame:
df_scale.na_dis$id <- id.list

#Put the MI columns back into the original data frame
df_scale$ress_distraction_mean_MI <- df_scale.na_dis$ress_distraction_mean_MI
df_scale$ress_distraction_sum_MI  <- df_scale.na_dis$ress_distraction_sum_MI
```
...
```{r}
df_1_orig <- subset(df_scale, select = c("ress_rumination_mean_orig", "ress_engagement_mean_orig", "ress_reappraisal_mean_orig", "ress_suppression_mean_orig", "ress_relaxation_mean_orig", "ress_distraction_mean_orig"))

df_1_MI <- subset(df_scale, select = c("ress_rumination_mean_MI", "ress_engagement_mean_MI" , "ress_reappraisal_mean_MI" , "ress_suppression_mean_MI" , "ress_relaxation_mean_MI" , "ress_distraction_mean_MI"))
```
...
qplot(df_scale$ress_rumination_mean_orig, geom="histogram") 
qplot(df_scale$ress_rumination_mean_MI, geom="histogram") 

```

Engagement subscale
```{r, warning = FALSE, message = FALSE}
qplot(df_scale$ress_engagement_mean_orig, geom="histogram") 
qplot(df_scale$ress_engagement_mean_MI, geom="histogram")

```
Reappraisal subscale
```{r, warning = FALSE, message = FALSE}

qplot(df_scale$ress_reappraisal_mean_orig, geom="histogram") 
qplot(df_scale$ress_reappraisal_mean_MI, geom="histogram") 

```
Suppression subscale
```{r, warning = FALSE, message = FALSE}

qplot(df_scale$ress_suppression_mean_orig, geom="histogram")
qplot(df_scale$ress_suppression_mean_MI, geom="histogram") 

```
Relaxation subscale
```{r, warning = FALSE, message = FALSE}

qplot(df_scale$ress_relaxation_mean_orig, geom="histogram")
qplot(df_scale$ress_relaxation_mean_MI, geom="histogram")

```
Distraction subscale
```{r, warning = FALSE, message = FALSE}

qplot(df_scale$ress_distraction_mean_orig, geom="histogram")
qplot(df_scale$ress_distraction_mean_MI, geom="histogram") 
