...
missing_summary <- stress_scale %>%
  summarise(across(everything(), ~ sum(is.na(.)))) %>%
  pivot_longer(cols = everything(),
               names_to = "variable",
               values_to = "n_missing") %>%
  mutate(pct_missing = round((n_missing / nrow(stress_scale)) * 100, 2)) %>%
  filter(n_missing > 0) %>%
...
num_items <- ncol(stress_scale.na)
...
for(i in 1:nrow(stress_scale.na)) {
  non_missing_count <- sum(!is.na(stress_scale.na[i, ]))
...
# Bring in the non-imputed mean and sum from stress_scale
stress_scale3 <- merge(stress_scale3,
                   stress_scale[, c("id", "stress_sum", "stress_mean")],
                   by = "id",
                   all.x = TRUE,
                   suffixes = c("", "_orig"))

# Reorder columns
stress_scale3 <- stress_scale3[, c("id", paste0("stress_", 1:10),
                           "stress_sum_orig", "stress_mean_orig",   # non-imputed
                           "stress_sum_MI", "stress_mean_MI")]      # imputed
```
...
describe(stress_scale3$stress_sum_orig)
